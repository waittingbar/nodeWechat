...
accessTokenJson = require('./access_token'), //引入本地存储的 access_token
      ...
      cardId = require('./cardId'), //引入卡券id
    cardInfo = require('./cardInfo2'), //引入卡券id
...
WeChat.prototype.getMenus = function () {
    var that = this;
    var url = util.format(that.apiURL.getMenus, that.apiDomain);
    that.requestGet(url).then(function (data) {
        var result = JSON.parse(data);
        console.log(result);
    });
 ...
 * @param {Response} res Response 对象
 */
WeChat.prototype.getCardInfo = function (req, res) {

    var that = this;
    this.getAccessToken().then(function (data) {
        //格式化请求连接
        var url = util.format(that.apiURL.getCardInfo, that.apiDomain, data);
        //使用 Post 请求
        that.requestPost(url, JSON.stringify(cardId)).then(function (data) {
            //将结果打印
            console.log(data);
            res.send(data);
        });
 ...
 * @param {Response} res Response 对象
 */
WeChat.prototype.setCardInfo = function (req, res) {

    var that = this;
    this.getAccessToken().then(function (data) {
        //格式化请求连接
        var url = util.format(that.apiURL.setCardInfo, that.apiDomain, data);
        //使用 Post 请求
        that.requestPost(url, JSON.stringify(cardInfo)).then(function (data) {
            //将结果打印
            console.log(data);
            res.send(data);
        });
 ...
 * @param {Response} res Response 对象
 */
WeChat.prototype.creatMenus = function (req, res) {
    console.log(2);
    var that = this;
    this.getAccessToken().then(function (data) {
        //格式化请求连接
